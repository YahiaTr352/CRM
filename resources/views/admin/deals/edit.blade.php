...
<div class="max-w-[1200px] mx-auto px-4 md:px-6 lg:px-8 pt-2 md:pt-4 lg:pt-6 space-y-8 pb-12">
        ...
        <div class="space-y-3">
            <div class="flex items-center gap-2">
                <div class="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-amber-50 border border-amber-100/50">
                    ...
                    <span class="text-[10px] font-bold text-amber-600 uppercase tracking-widest">Edit Opportunity</span>
                </div>
            ...
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-3xl font-black text-slate-900 tracking-tight">Modify Deal Details</h1>
                    <p class="text-slate-500 font-medium text-sm mt-1 max-w-xl leading-relaxed">
                        Update the parameters for <span class="text-indigo-600 font-bold">"{{ $deal->deal_name }}"</span> to ensure pipeline accuracy.
                    </p>
                </div>
            </div>
        </div>
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 animate-in fade-in slide-in-from-bottom-4 duration-1000 delay-75">
        <div class="flex items-center justify-between mb-6">
            <h3 class="text-[11px] font-black text-slate-400 uppercase tracking-widest">Deal Lifecycle Progress</h3>
            <span class="px-2.5 py-1 rounded-lg bg-indigo-50 text-[10px] font-black text-indigo-600 uppercase tracking-widest">
                Current: {{ $deal->stage->name ?? 'Unassigned' }}
            </span>
        </div>
            ...
            <div class="relative flex justify-between">
                @foreach($stages as $id => $name)
                    ...
                    <div class="flex flex-col items-center gap-3 group relative">
                        <div class="w-8 h-8 rounded-full flex items-center justify-center z-10 transition-all duration-500 {{ $isCompleted ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-200' : 'bg-white border-2 border-slate-200 text-slate-400' }} {{ $isCurrent ? 'ring-4 ring-indigo-50' : '' }}">
                        ...
                        <span class="text-[9px] font-black uppercase tracking-tighter transition-colors {{ $isCompleted ? 'text-indigo-600' : 'text-slate-400' }}">{{ $name }}</span>
                    </div>
                @endforeach
            </div>
        </div>
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-1000 delay-150">
        <div class="p-6 md:p-8 border-b border-slate-100 bg-slate-50/30 flex items-center justify-between">
            <div>
                <h2 class="text-lg font-bold text-slate-900">Specifications Update</h2>
                <p class="text-slate-500 text-sm mt-1 font-medium">Refine the core attributes of this sales opportunity.</p>
            </div>
            <div class="text-right">
                <span class="text-[10px] font-black text-slate-400 uppercase tracking-widest block">Last Updated</span>
                <span class="text-xs font-bold text-slate-600">{{ $deal->updated_at ? $deal->updated_at->format('M d, Y • H:i') : 'N/A' }}</span>
            </div>
        </div>

        <form method="POST" action="{{ route("admin.deals.update", [$deal->id]) }}" enctype="multipart/form-data" class="p-6 md:p-8 space-y-10">
            @method('PUT')
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-10">
                ...
                <div class="space-y-3">
                    ...
                    <div class="relative group">
                        ...
                        <input class="w-full h-12 pl-11 pr-4 bg-slate-50/50 border border-slate-200 rounded-xl text-sm font-bold text-slate-900 placeholder:text-slate-400 placeholder:font-medium focus:outline-none focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 focus:bg-white transition-all {{ $errors->has('deal_name') ? 'border-rose-300 bg-rose-50/30' : '' }}" type="text" name="deal_name" id="deal_name" value="{{ old('deal_name', $deal->deal_name) }}" placeholder="e.g. Enterprise Solution Q4" required>
                    </div>
                ...
                <div class="space-y-3">
                    ...
                    <div class="relative group">
                        ...
                        <input class="w-full h-12 pl-9 pr-4 bg-slate-50/50 border border-slate-200 rounded-xl text-sm font-black text-slate-900 focus:outline-none focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-500 focus:bg-white transition-all {{ $errors->has('amount') ? 'border-rose-300 bg-rose-50/30' : '' }}" type="number" name="amount" id="amount" value="{{ old('amount', $deal->amount) }}" step="0.01" placeholder="0.00">
                    </div>
                ...
                <div class="space-y-3">
                    ...
                    <div class="relative group">
                        ...
                        <input class="w-full h-12 pl-11 pr-4 bg-slate-50/50 border border-slate-200 rounded-xl text-sm font-bold text-slate-900 focus:outline-none focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 focus:bg-white transition-all date {{ $errors->has('closing_date') ? 'is-invalid' : '' }}" type="text" name="closing_date" id="closing_date" value="{{ old('closing_date', $deal->closing_date) }}" placeholder="Select expected close date">
                    </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-12 pt-4">
                    ...
                    <div class="needsclick dropzone rounded-2xl border-2 border-dashed border-slate-200 hover:border-indigo-400 hover:bg-indigo-50/30 transition-all {{ $errors->has('attachments') ? 'border-rose-300' : '' }}" id="attachments-dropzone">
                        <div class="dz-message" data-dz-message>
                            <div class="flex flex-col items-center justify-center py-6">
                                ...
                                <p class="text-sm font-bold text-slate-600">Drop files or click to upload</p>
                                <p class="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-2">Max Size: 20MB</p>
                            </div>
                        </div>
                    </div>
                ...
                <div class="space-y-3">
                    ...
                    <div class="flex items-center gap-2 mb-3">
                        <button type="button" class="select-all px-3 py-1.5 bg-slate-100 rounded-lg text-[10px] font-black text-slate-500 uppercase tracking-widest hover:bg-indigo-600 hover:text-white transition-all">Select All</button>
                        <button type="button" class="deselect-all px-3 py-1.5 bg-slate-100 rounded-lg text-[10px] font-black text-slate-500 uppercase tracking-widest hover:bg-rose-600 hover:text-white transition-all">Deselect All</button>
                    </div>
            ...
            <div class="pt-10 border-t border-slate-100 flex items-center justify-end gap-6">
                <a href="{{ route('admin.deals.index') }}" class="text-sm font-bold text-slate-400 hover:text-slate-600 transition-colors">
                    Discard Changes
                </a>
                <button class="inline-flex items-center gap-2.5 px-8 py-4 bg-slate-900 rounded-xl text-sm font-bold text-white shadow-xl shadow-slate-200 hover:bg-indigo-600 hover:-translate-y-0.5 hover:shadow-indigo-200 transition-all active:scale-95" type="submit">
                    <i class="fas fa-save"></i>
                    <span>Apply Permanent Changes</span>
                </button>
            </div>
        </form>
    </div>
