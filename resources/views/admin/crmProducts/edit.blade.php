        ...
        <div class="space-y-3">
            <div class="flex items-center gap-2">
                <div class="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-indigo-50 border border-indigo-100/50">
                    ...
                    <span class="text-[10px] font-bold text-indigo-600 uppercase tracking-widest">Update Asset</span>
                </div>
            ...
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-3xl font-black text-slate-900 tracking-tight">Modify Product</h1>
                    <p class="text-slate-500 font-medium text-sm mt-1 max-w-xl leading-relaxed">
                        Refining specifications for <span class="text-indigo-600 font-bold">"{{ $crmProduct->product_name }}"</span>.
                    </p>
                </div>
            </div>
        </div>
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-1000 delay-150">
        <div class="p-6 md:p-8 border-b border-slate-100 bg-slate-50/30 flex items-center justify-between">
            <div>
                ...
                <p class="text-slate-500 text-sm mt-1 font-medium">Refine asset identity and commercial valuation.</p>
            </div>
            <div class="flex items-center gap-3">
                <div class="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-white border border-slate-200 shadow-sm">
                    ...
                    <div class="form-check p-0 min-h-0 flex items-center">
                        ...
                        <label class="relative inline-flex items-center cursor-pointer">
                            <input type="checkbox" name="product_active" id="product_active" value="1" {{ $crmProduct->product_active || old('product_active', 0) === 1 ? 'checked' : '' }} class="sr-only peer" form="product-form">
        ...
        <form id="product-form" method="POST" action="{{ route("admin.crm-products.update", [$crmProduct->id]) }}" enctype="multipart/form-data" class="p-6 md:p-8 space-y-10">
            @method('PUT')
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-10">
                    ...
                    <div class="needsclick dropzone rounded-2xl border-2 border-dashed border-slate-200 hover:border-indigo-400 hover:bg-indigo-50/30 transition-all {{ $errors->has('product_image') ? 'border-rose-300 bg-rose-50/30' : '' }}" id="product_image-dropzone">
                        <div class="dz-message" data-dz-message>
                            <div class="flex flex-col items-center justify-center py-4">
                                ...
                                <p class="text-sm font-bold text-slate-600">Update asset image or drag and drop</p>
                                <p class="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-2">Max Size: 5MB</p>
                            </div>
                        </div>
                    </div>
                ...
                <div class="space-y-3">
                    ...
                    <div class="relative group">
                        ...
                        <input class="w-full h-12 pl-11 pr-4 bg-slate-50/50 border border-slate-200 rounded-xl text-sm font-bold text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 focus:bg-white transition-all {{ $errors->has('product_name') ? 'border-rose-300 bg-rose-50/30' : '' }}" type="text" name="product_name" id="product_name" value="{{ old('product_name', $crmProduct->product_name) }}" required>
                    </div>
                ...
                <div class="space-y-3">
                    ...
                    <select class="form-control select2 {{ $errors->has('product_category') ? 'is-invalid' : '' }}" name="product_category_id" id="product_category_id">
                        @foreach($product_categories as $id => $entry)
                            <option value="{{ $id }}" {{ (old('product_category_id') ? old('product_category_id') : $crmProduct->product_category->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                        @endforeach
                    </select>
                ...
                <div class="space-y-3">
                    ...
                    <div class="relative group">
                        ...
                        <input class="w-full h-12 pl-9 pr-4 bg-slate-50/50 border border-slate-200 rounded-xl text-sm font-black text-slate-900 focus:outline-none focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-500 focus:bg-white transition-all {{ $errors->has('unit_price') ? 'border-rose-300 bg-rose-50/30' : '' }}" type="number" name="unit_price" id="unit_price" value="{{ old('unit_price', $crmProduct->unit_price) }}" step="0.01">
                    </div>
                ...
                <div class="space-y-3 md:col-span-2">
                    ...
                    <div class="rounded-2xl border border-slate-200 overflow-hidden shadow-sm">
                        <textarea class="form-control ckeditor {{ $errors->has('description') ? 'is-invalid' : '' }}" name="description" id="description">{!! old('description', $crmProduct->description) !!}</textarea>
                    </div>
            ...
            <div class="pt-10 border-t border-slate-100 flex items-center justify-end gap-6">
                ...
                <button class="inline-flex items-center gap-2.5 px-8 py-4 bg-slate-900 rounded-xl text-sm font-bold text-white shadow-xl shadow-slate-200 hover:bg-indigo-600 hover:-translate-y-0.5 transition-all active:scale-95" type="submit">
