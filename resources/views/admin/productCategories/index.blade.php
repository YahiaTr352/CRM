...
<div class="max-w-[1200px] mx-auto px-4 md:px-6 lg:px-8 pt-2 md:pt-4 lg:pt-6 space-y-8">
    ...
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-8 border-b border-slate-200/60 animate-in fade-in slide-in-from-top-4 duration-700">
        <div class="space-y-3">
            <div class="flex items-center gap-2">
                <div class="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-indigo-50 border border-indigo-100/50">
                    ...
                    <span class="text-[10px] font-bold text-indigo-600 uppercase tracking-widest">Global Taxonomy</span>
                </div>
            ...
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-3xl font-black text-slate-900 tracking-tight">{{ trans('cruds.productCategory.title') }}</h1>
                    <p class="text-slate-500 font-medium text-sm mt-1 max-w-xl">
                        Strategically organize and classify your product portfolio for optimal oversight.
                    </p>
                </div>
            </div>
        </div>
        ...
        <div class="flex items-center gap-3">
            @can('product_category_create')
                ...
                <a href="{{ route('admin.product-categories.create') }}" class="inline-flex items-center gap-2.5 px-6 py-2.5 bg-indigo-600 rounded-xl text-sm font-bold text-white shadow-lg shadow-indigo-100 hover:bg-indigo-700 hover:-translate-y-0.5 transition-all active:scale-95">
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-1000 delay-150">
        <div class="p-6 md:p-8 border-b border-slate-100 bg-slate-50/30 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
                <h2 class="text-lg font-bold text-slate-900">Category Matrix</h2>
                <p class="text-slate-500 text-sm mt-1 font-medium text-slate-400">Inventory of operational product classifications.</p>
            </div>
            <div class="flex items-center gap-2">
                <div class="relative group">
                    ...
                    <input type="text" id="categorySearch" placeholder="Filter categories..." class="pl-10 pr-4 py-2 bg-white border border-slate-200 rounded-xl text-sm font-bold text-slate-900 focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 w-full sm:w-64 transition-all">
                </div>
            </div>
        </div>

        <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse datatable-ProductCategory">
                <tbody class="divide-y divide-slate-100">
                    @foreach($productCategories as $key => $productCategory)
                        <tr data-entry-id="{{ $productCategory->id }}" class="hover:bg-slate-50/50 transition-all group">
                            <td class="px-8 py-5">
                            ...
                            <td class="px-8 py-5">
                                <div class="flex items-center gap-4">
                                    ...
                                    <div>
                                        <span class="text-sm font-bold text-slate-900 block leading-tight group-hover:text-indigo-600 transition-colors">{{ $productCategory->name ?? '' }}</span>
                            ...
                            <td class="px-8 py-5 text-right">
                                <div class="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                                    @can('product_category_show')
                                        <a href="{{ route('admin.product-categories.show', $productCategory->id) }}" class="w-9 h-9 rounded-xl flex items-center justify-center text-slate-400 bg-white border border-slate-100 shadow-sm hover:text-indigo-600 hover:border-indigo-100 hover:shadow-md transition-all active:scale-90" title="Review Intelligence">
                                            <i class="fas fa-eye text-xs"></i>
                                        </a>
                                    @endcan
                                    @can('product_category_edit')
                                        <a href="{{ route('admin.product-categories.edit', $productCategory->id) }}" class="w-9 h-9 rounded-xl flex items-center justify-center text-slate-400 bg-white border border-slate-100 shadow-sm hover:text-blue-600 hover:border-blue-100 hover:shadow-md transition-all active:scale-90" title="Refine Configuration">
                                            <i class="fas fa-edit text-xs"></i>
                                        </a>
                                    @endcan
                                    @can('product_category_delete')
                                        <form action="{{ route('admin.product-categories.destroy', $productCategory->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" class="inline-block">
                                            <input type="hidden" name="_method" value="DELETE">
                                            <input type="hidden" name="_token" value="{{ csrf_token() }}">
                                            <button type="submit" class="w-9 h-9 rounded-xl flex items-center justify-center text-slate-400 bg-white border border-slate-100 shadow-sm hover:text-rose-600 hover:border-rose-100 hover:shadow-md transition-all active:scale-90" title="Terminate Category">
