...
<div class="max-w-[1200px] mx-auto px-4 md:px-6 lg:px-8 pt-2 md:pt-4 lg:pt-6 space-y-8 pb-12">
    ...
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-8 border-b border-slate-200/60 animate-in fade-in slide-in-from-top-4 duration-700">
        <div class="space-y-3">
            ...
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-3xl font-black text-slate-900 tracking-tight">Modify Contact</h1>
                    <p class="text-slate-500 font-medium text-sm mt-1 max-w-xl leading-relaxed">
                        Updating profile for <span class="text-indigo-600 font-bold">"{{ $crmContact->contact_first_name }} {{ $crmContact->contact_last_name }}"</span>.
                    </p>
                </div>
            </div>
        </div>
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-1000 delay-150">
        <div class="p-6 md:p-8 border-b border-slate-100 bg-slate-50/30 flex items-center justify-between">
            <div>
                <h2 class="text-lg font-bold text-slate-900">Personal & Corporate Profile</h2>
                <p class="text-slate-500 text-sm mt-1 font-medium">Refine identification and organizational details.</p>
            </div>
            <div class="text-right">
                <span class="text-[10px] font-black text-slate-400 uppercase tracking-widest block">Last Sync</span>
                <span class="text-xs font-bold text-slate-600">{{ $crmContact->updated_at ? $crmContact->updated_at->format('M d, Y • H:i') : 'N/A' }}</span>
            </div>
        </div>

        <form method="POST" action="{{ route("admin.crm-contacts.update", [$crmContact->id]) }}" enctype="multipart/form-data" class="p-6 md:p-8 space-y-10">
            @method('PUT')
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-10">
                ...
                <div class="space-y-3 md:col-span-2">
                    ...
                    <div class="relative group">
                        ...
                        <input class="w-full h-12 pl-11 pr-4 bg-slate-50/50 border border-slate-200 rounded-xl text-sm font-bold text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 focus:bg-white transition-all {{ $errors->has('company') ? 'border-rose-300 bg-rose-50/30' : '' }}" type="text" name="company" id="company" value="{{ old('company', $crmContact->company) }}" placeholder="Organization name">
                    </div>
                ...
                <div class="space-y-3">
                    ...
                    <div class="relative group">
                        ...
                        <input class="w-full h-12 pl-11 pr-4 bg-slate-50/50 border border-slate-200 rounded-xl text-sm font-bold text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 focus:bg-white transition-all {{ $errors->has('contact_first_name') ? 'border-rose-300 bg-rose-50/30' : '' }}" type="text" name="contact_first_name" id="contact_first_name" value="{{ old('contact_first_name', $crmContact->contact_first_name) }}" required>
                    </div>
                ...
                <div class="space-y-3">
                    ...
                    <div class="relative group">
                        ...
                        <input class="w-full h-12 pl-11 pr-4 bg-slate-50/50 border border-slate-200 rounded-xl text-sm font-bold text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 focus:bg-white transition-all {{ $errors->has('contact_last_name') ? 'border-rose-300 bg-rose-50/30' : '' }}" type="text" name="contact_last_name" id="contact_last_name" value="{{ old('contact_last_name', $crmContact->contact_last_name) }}">
                    </div>
                ...
                <div class="space-y-3">
                    ...
                    <div class="relative group">
                        ...
                        <input class="w-full h-12 pl-11 pr-4 bg-slate-50/50 border border-slate-200 rounded-xl text-sm font-bold text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 focus:bg-white transition-all {{ $errors->has('contact_phone_1') ? 'border-rose-300 bg-rose-50/30' : '' }}" type="text" name="contact_phone_1" id="contact_phone_1" value="{{ old('contact_phone_1', $crmContact->contact_phone_1) }}">
                    </div>
                ...
                <div class="space-y-3">
                    ...
                    <div class="relative group">
                        ...
                        <input class="w-full h-12 pl-11 pr-4 bg-slate-50/50 border border-slate-200 rounded-xl text-sm font-bold text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 focus:bg-white transition-all {{ $errors->has('contact_address') ? 'border-rose-300 bg-rose-50/30' : '' }}" type="text" name="contact_address" id="contact_address" value="{{ old('contact_address', $crmContact->contact_address) }}" placeholder="Full business address">
                    </div>
                ...
                <div class="space-y-3 md:col-span-2">
                    ...
                    <div class="relative group">
                        ...
                        <textarea class="w-full pl-11 pr-4 py-3 bg-slate-50/50 border border-slate-200 rounded-xl text-sm font-bold text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 focus:bg-white transition-all min-h-[120px] {{ $errors->has('contact_description') ? 'border-rose-300 bg-rose-50/30' : '' }}" name="contact_description" id="contact_description" placeholder="Strategic notes or relationship context...">{{ old('contact_description', $crmContact->contact_description) }}</textarea>
                    </div>
            ...
            <div class="pt-10 border-t border-slate-100 flex items-center justify-end gap-6">
                ...
                <button class="inline-flex items-center gap-2.5 px-8 py-4 bg-indigo-600 rounded-xl text-sm font-bold text-white shadow-xl shadow-indigo-200 hover:bg-indigo-700 hover:-translate-y-0.5 transition-all active:scale-95" type="submit">
                    ...
                    <span>Securely Update Contact</span>
                </button>
            </div>
        </form>
    </div>
