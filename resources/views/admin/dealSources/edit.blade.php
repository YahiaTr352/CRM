...
<div class="max-w-[800px] mx-auto px-4 md:px-6 lg:px-8 pt-2 md:pt-4 lg:pt-6 space-y-8">
    ...
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-8 border-b border-slate-200/60 animate-in fade-in slide-in-from-top-4 duration-700">
        <div class="space-y-3">
            ...
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-3xl font-black text-slate-900 tracking-tight">Refine Source Intelligence</h1>
                    <p class="text-slate-500 font-medium text-sm mt-1 max-w-xl">
                        Updating the <span class="text-indigo-600 font-bold">{{ $dealSource->name }}</span> acquisition channel specifications.
                    </p>
                </div>
            </div>
        </div>
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-1000 delay-150">
        <div class="p-6 md:p-8 border-b border-slate-100 bg-slate-50/30">
            <h2 class="text-lg font-bold text-slate-900">Source Refinement</h2>
            <p class="text-slate-500 text-sm mt-1 font-medium">Modify core specifications for acquisition channel classification.</p>
        </div>

        <form method="POST" action="{{ route("admin.deal-sources.update", [$dealSource->id]) }}" enctype="multipart/form-data" class="p-6 md:p-8 space-y-8">
            @method('PUT')
            @csrf
            ...
            <div class="space-y-3">
                ...
                <div class="relative group">
                    ...
                    <input class="w-full pl-11 pr-4 py-3 bg-white border border-slate-200 rounded-xl text-sm font-bold text-slate-900 placeholder:text-slate-400 placeholder:font-medium focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 transition-all {{ $errors->has('name') ? 'border-rose-300 bg-rose-50/30' : '' }}" type="text" name="name" id="name" value="{{ old('name', $dealSource->name) }}" placeholder="Enter source name" required>
                </div>
                @if($errors->has('name'))
