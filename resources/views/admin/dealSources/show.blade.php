    ...
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-8 border-b border-slate-200/60 animate-in fade-in slide-in-from-top-4 duration-700">
        <div class="space-y-3">
            <div class="flex items-center gap-2">
                ...
                <a href="{{ route('admin.deal-sources.index') }}" class="text-[10px] font-bold text-slate-400 hover:text-indigo-600 uppercase tracking-widest transition-colors">Source Matrix</a>
            </div>
            ...
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-3xl font-black text-slate-900 tracking-tight">Source Details</h1>
                    <p class="text-slate-500 font-medium text-sm mt-1 max-w-xl leading-relaxed">
                        Comprehensive overview of the <span class="text-indigo-600 font-bold">{{ $dealSource->name }}</span> lead acquisition channel.
                    </p>
                </div>
            </div>
        </div>
        ...
        <div class="flex items-center gap-3">
            @can('deal_source_edit')
                <a href="{{ route('admin.deal-sources.edit', $dealSource->id) }}" class="inline-flex items-center gap-2.5 px-5 py-2.5 bg-white border border-slate-200 rounded-xl text-sm font-bold text-slate-600 shadow-sm hover:border-indigo-200 hover:text-indigo-600 transition-all active:scale-95">
                    <i class="fas fa-edit text-slate-400"></i>
                    <span>Edit Source</span>
                </a>
            @endcan
            <a href="{{ route('admin.deal-sources.index') }}" class="inline-flex items-center gap-2.5 px-5 py-2.5 bg-slate-900 rounded-xl text-sm font-bold text-white shadow-lg shadow-slate-200 hover:bg-slate-800 transition-all active:scale-95">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 animate-in fade-in slide-in-from-bottom-4 duration-1000 delay-150">
        ...
        <div class="lg:col-span-2 space-y-8">
            ...
            <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                ...
                <div class="p-6 md:p-8">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-8">
                        <div class="space-y-1">
                            <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest">{{ trans('cruds.dealSource.fields.name') }}</p>
                            <div class="flex items-center gap-3 pt-1">
                                ...
                                <p class="text-base font-bold text-slate-900">{{ $dealSource->name }}</p>
                            </div>
                        </div>
                        <div class="space-y-1">
                            <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Internal Identifier</p>
                            <p class="text-base font-mono font-bold text-indigo-600 pt-1">SRC-{{ str_pad($dealSource->id, 5, '0', STR_PAD_LEFT) }}</p>
                        </div>
                    </div>
                </div>
            </div>

            <!-- Optional: Channel Performance Placeholder -->
            <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden p-12 text-center">
                ...
                <h3 class="text-sm font-bold text-slate-900 uppercase tracking-widest">Channel Performance</h3>
                <p class="text-xs text-slate-400 mt-1">Cross-referencing acquisition metrics for this source.</p>
            </div>
        </div>
            ...
            <div class="bg-slate-900 rounded-2xl p-6 text-white shadow-xl shadow-slate-200">
                <div class="space-y-4">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            <p class="text-[9px] font-black text-white/40 uppercase tracking-widest">Source Timeline</p>
                            <p class="text-xs font-bold">Acquisition Record</p>
                        </div>
                    </div>
                    
                    <div class="grid grid-cols-1 gap-4 pt-4 border-t border-white/10">
                        <div>
                            <p class="text-[9px] font-black text-white/40 uppercase tracking-widest mb-1">{{ trans('cruds.dealSource.fields.created_at') }}</p>
                            <p class="text-[11px] font-bold">{{ $dealSource->created_at ? $dealSource->created_at->format('M d, Y') : 'N/A' }}</p>
                        </div>
                    </div>

                    @can('deal_source_delete')
                        <form action="{{ route('admin.deal-sources.destroy', $dealSource->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" class="pt-4">
