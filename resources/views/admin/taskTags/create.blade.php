...
<div class="max-w-[800px] mx-auto px-4 md:px-6 lg:px-8 pt-2 md:pt-4 lg:pt-6 space-y-8">
    ...
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-8 border-b border-slate-200/60 animate-in fade-in slide-in-from-top-4 duration-700">
        <div class="space-y-3">
            <div class="flex items-center gap-2">
                <div class="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-emerald-50 border border-emerald-100/50">
                    ...
                    <span class="text-[10px] font-bold text-emerald-600 uppercase tracking-widest">New Tag Generation</span>
                </div>
            ...
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-3xl font-black text-slate-900 tracking-tight">Generate New Tag</h1>
                    <p class="text-slate-500 font-medium text-sm mt-1 max-w-xl">
                        Define a new categorization label to refine your workflow intelligence.
                    </p>
                </div>
            </div>
        </div>
        ...
        <div class="flex items-center gap-3">
            <a href="{{ route('admin.task-tags.index') }}" class="inline-flex items-center gap-2.5 px-5 py-2.5 bg-white border border-slate-200 rounded-xl text-sm font-bold text-slate-600 shadow-sm hover:border-slate-300 hover:text-slate-900 transition-all active:scale-95">
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-1000 delay-150">
        <div class="p-6 md:p-8 border-b border-slate-100 bg-slate-50/30">
            <h2 class="text-lg font-bold text-slate-900">Tag Specifications</h2>
            <p class="text-slate-500 text-sm mt-1 font-medium">Core identifier for task categorization.</p>
        </div>

        <form method="POST" action="{{ route("admin.task-tags.store") }}" enctype="multipart/form-data" class="p-6 md:p-8 space-y-8">
            @csrf
            ...
            <div class="space-y-2">
                ...
                @endif
                @if(trans('cruds.taskTag.fields.name_helper'))
                    <p class="text-[10px] text-slate-400 font-medium ml-1 italic">{{ trans('cruds.taskTag.fields.name_helper') }}</p>
                @endif
            </div>

            <!-- Submit Section -->
            <div class="pt-8 border-t border-slate-100 flex items-center justify-end gap-4">
                <a href="{{ route('admin.task-tags.index') }}" class="px-6 py-3 text-sm font-bold text-slate-500 hover:text-slate-800 transition-colors">
                    Discard Changes
                </a>
