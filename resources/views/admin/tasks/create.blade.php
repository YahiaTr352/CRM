...
<div class="max-w-[1200px] mx-auto px-4 md:px-6 lg:px-8 pt-2 md:pt-4 lg:pt-6 space-y-8">
        ...
        <div class="space-y-3">
            <div class="flex items-center gap-2">
                <div class="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-emerald-50 border border-emerald-100/50">
                    ...
                    <span class="text-[10px] font-bold text-emerald-600 uppercase tracking-widest">Action Required</span>
                </div>
            ...
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-3xl font-black text-slate-900 tracking-tight">Create New Task</h1>
                    <p class="text-slate-500 font-medium text-sm mt-1 max-w-xl">
                        Define objectives, set priorities, and assign responsibilities.
                    </p>
                </div>
            </div>
        </div>
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-1000 delay-150">
        <div class="p-6 md:p-8 border-b border-slate-100 bg-slate-50/30">
            <h2 class="text-lg font-bold text-slate-900">Task Specifications</h2>
            <p class="text-slate-500 text-sm mt-1 font-medium">Core details and execution parameters.</p>
        </div>

        <form method="POST" action="{{ route("admin.tasks.store") }}" enctype="multipart/form-data" class="p-6 md:p-8 space-y-8">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div class="space-y-4">
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        @foreach(App\Models\Task::PRIORITY_RADIO as $key => $label)
                            ...
                            @endphp
                            <label class="relative flex items-center justify-center p-3 rounded-xl border-2 cursor-pointer transition-all hover:bg-slate-50 {{ old('priority', 'low') === $key ? 'border-'.$pColor.'-500 bg-'.$pColor.'-50/30' : 'border-slate-100' }}">
                                <input type="radio" name="priority" value="{{ $key }}" class="hidden" {{ old('priority', 'low') === (string) $key ? 'checked' : '' }}>
                                <span class="text-xs font-black uppercase tracking-tight text-{{ old('priority', 'low') === $key ? $pColor.'-700' : 'slate-500' }}">{{ $label }}</span>
                            </label>
                        @endforeach
                    </div>
                </div>
                ...
                <div class="space-y-2">
                    ...
                    <div class="relative group">
                        ...
                        <input class="w-full pl-11 pr-4 py-3 bg-white border border-slate-200 rounded-xl text-sm font-bold text-slate-900 date" type="text" name="due_date" id="due_date" value="{{ old('due_date') }}" placeholder="Select due date">
                    </div>
                </div>
